...
void Video::LoadVideo(const char *fname)
{
	...
	screen = myApp->win->window;
	texture = SDL_CreateTexture(myApp->render->renderer, SDL_PIXELFORMAT_IYUV, SDL_TEXTUREACCESS_STREAMING, video->width, video->height);
...
void Video::PlayVideo(const char *fname, SDL_Rect r,bool isLoop)
{
	// Loading video ---------------------
	ResetValues();
	rendering_rect = r;
...
bool Video::Update(float dt)
{
	if (want_to_play && !quit && THEORAPLAY_isDecoding(decoder))
	{
		...
		SDL_RenderCopy(myApp->render->renderer, texture, NULL, &rendering_rect);
	}

	if (quit)
		ResetValues();
	if (loop&&IsPlaying()==false&&want_to_play==true) {
